...
Some of the settings were set up in the first code chunk, applying for all the plots generated. A column and density plot are drawn to compare between different years and response groups.

It can be concluded from the plot that the overall patterns of data value distribution for response are quite similar in the two years chosen. And for different resonse groups, the "poor" group possesses a significantly different density and distribution compared with other groups, which tend to overlap with each other on the density plot.
